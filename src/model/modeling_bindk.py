...
from src.tokenizer.modeling_tokenizer import BINDKTokenizer, SentenceTokenizer
...
class BINDK(nn.Module):
    ...
    def set_tokenizer(self, bind_tokenizer: BINDKTokenizer):
...
class LitBINDK(L.LightningModule):
    def __init__(
        self,
        ...
        bindk_tokenizer = BINDKTokenizer(base_tokenizer_name=base_model_name)
        self.bindk.set_tokenizer(bindk_tokenizer)
    ...
    def forward(self, batch, task, pred):
        if task=='jamo':
            loss, logits, pred_ids, sentence_denoised = self.bindk.forward(
            ...
            loss_char, logits, pred_ids, sentence_denoised = self.bindk.forward(
                sentence_noisy=batch['sentence_noisy'],
                sentence=batch['sentence'],
                task='char',
                pred=True
            )
            loss_jamo, logits, pred_ids, sentence_denoised = self.bindk.forward(
