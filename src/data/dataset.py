...
def get_dacon_train_dataloader(dataset_name, batch_size, max_length):
    ds = datasets.load_dataset(dataset_name)
    train_ds = ds['train']
    return DataLoader(train_ds, batch_size=batch_size, collate_fn=TrainValCollateFn(max_length=max_length, mode='train'))

def get_dacon_dev_dataloader(dataset_name, batch_size, max_length):
    ds = datasets.load_dataset(dataset_name)
    dev_ds = ds['dev']
    return DataLoader(dev_ds, batch_size=batch_size, collate_fn=TrainValCollateFn(max_length=max_length, mode='valid'))

def get_kor_leaner_train_dataloader(dataset_name, **kwargs):
    ds = datasets.load_dataset(dataset_name)
    train_ds = ds['train']
    return DataLoader(train_ds, **kwargs)

def get_kor_learner_dev_test_dataloader(dataset_name, **kwargs):
    ds = datasets.load_dataset(dataset_name)
    dev_ds, test_ds = ds['dev'], ds['test']
    return DataLoader(dev_ds, **kwargs), DataLoader(test_ds, **kwargs)

def get_kor_union_train_dataloader(dataset_name, **kwargs):
    ds = datasets.load_dataset(dataset_name)
    train_ds = ds['train']
    return DataLoader(train_ds, **kwargs)

def get_kor_union_dev_test_dataloader(dataset_name, **kwargs):
    ds = datasets.load_dataset(dataset_name)
    dev_ds, test_ds = ds['dev'], ds['test']
    return DataLoader(dev_ds, **kwargs), DataLoader(test_ds, **kwargs)

def get_kor_native_train_dataloader(dataset_name, **kwargs):
    ds = datasets.load_dataset(dataset_name)
    train_ds = ds['train']
    return DataLoader(train_ds, **kwargs)

def get_kor_native_dev_test_dataloader(dataset_name, **kwargs):
    ds = datasets.load_dataset(dataset_name)
    dev_ds, test_ds = ds['dev'], ds['test']
    return DataLoader(dev_ds, **kwargs), DataLoader(test_ds, **kwargs)